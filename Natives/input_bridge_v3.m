...
// Lazily init GLFW bridge (adapted from Taylen-chud/Amethyst-iOS).
// Called on first use rather than from JNI_OnLoad because JNI_OnLoad fires
// while org.lwjgl.system.MemoryUtil is mid-init on Java 25, and eager
// FindClass+GetStaticMethodID triggers a circular class-init SIGSEGV.
void ensureGLFWBridge() {
    ...
    logPendingException(env, "ensureGLFWBridge: FindClass(GLFW) threw");
    if (localGlfwClass == NULL) {
        NSLog(@"ensureGLFWBridge: FindClass failed, retrying via system classloader");
        ...
        if (clCls != NULL && classCls != NULL) {
            ...
            logPendingException(env, "ensureGLFWBridge: system classloader fallback threw");
        } else {
            (*env)->ExceptionClear(env);
        }
    }
    if (localGlfwClass == NULL) {
        NSLog(@"ensureGLFWBridge: FindClass(org/lwjgl/glfw/GLFW) failed, aborting GLFW native init");
        return;
    }
    ...
    if (method_internalWindowSizeChanged == NULL) {
        (*env)->ExceptionDescribe(env);
        (*env)->ExceptionClear(env);
        NSLog(@"ensureGLFWBridge: GetStaticMethodID(internalWindowSizeChanged) failed");
    }
    ...
    if (field_keyDownBuffer == NULL) {
        (*env)->ExceptionDescribe(env);
        (*env)->ExceptionClear(env);
        NSLog(@"ensureGLFWBridge: GetStaticFieldID(keyDownBuffer) failed, aborting GLFW native init");
        return;
    }
...
jint JNI_OnLoad(JavaVM* vm, void* reserved) {
    ...
    // (Java 25 class-init deadlock → SIGSEGV in get_method_id).
...
void handleFramebufferSizeJava(void* window, int w, int h) {
    ...
    if (vmGlfwClass && method_internalWindowSizeChanged) {
        JNIEnv *env = getThreadJNIEnv();
        if (env) (*env)->CallStaticVoidMethod(env, vmGlfwClass, method_internalWindowSizeChanged, (long)window, w, h);
    }
}
...
void CallbackBridge_nativeSendKey(int key, int scancode, int action, int mods) {
    ensureGLFWBridge();
    if (GLFW_invoke_Key && isInputReady && keyDownBuffer) {
